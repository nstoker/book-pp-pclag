...
func TestRun(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expErr != nil {
				if err == nil {
					t.Errorf("%s - Expected error. Got nil instead", tc.name)
				}

				if !errors.Is(err, tc.expErr) {
					t.Errorf("%s - Expected error %q, got %q instead", tc.name, tc.exp, err)
				}
			...
			if res.String() != tc.exp {
				t.Errorf("%s - Expected %q, got %q instead", tc.name, tc.exp, &res)
			}
		})
	}
}
